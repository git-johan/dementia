...
@router.post(
    ...
    summary="Process speech audio file",
    description="Upload audio file for direct speech-to-text processing with Norwegian NB-Whisper"
)
async def process_speech(
    ...
    """Process audio file for speech recognition directly.

    This endpoint accepts audio files from device recording apps and processes
    them immediately using the globally loaded Norwegian NB-Whisper model.
    ...
    Returns:
        Direct transcription result with Norwegian text
    """
    try:
        logger.info(f"Processing speech file directly: {file.filename}")
        ...
        result = await process_audio_direct(audio_content, metadata_dict, model_size)
...
async def process_audio_direct(audio_content: bytes, metadata: dict, model_size: str) -> SpeechTranscriptionResponse:
    """Process audio content directly using the selected model."""
...
@router.get(
    ...
    description="Get information about the loaded NB-Whisper processor"
)
async def get_processor_capabilities():
    """Get capabilities of all loaded speech processors.

    Returns:
        Information about all available NB-Whisper models
    """
    try:
        ...
        # Build capabilities for each loaded model
        processors = []
        for model_size in available_model_sizes:
            if model_size in nb_whisper_models:
                ...
                processors.append(capabilities)
        ...
        return {
            ...
            "status": "ready",
            "processing_mode": "Multi-model direct processing",
            "gpu_acceleration": gpu_status,
            "usage": "Add model_size parameter to /api/process/speech (tiny/base/small/medium/large)"
        }

    except Exception as e:
        logger.error(f"Failed to get processor capabilities: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
