...
class ProcessorResult(BaseModel):
    """Result from a single speech processor."""

    processor: str = Field(..., description="Processor name", example="nb-whisper-large")
    ...
    segments: Optional[List[Dict[str, Any]]] = Field(
        None,
        description="Detailed segments with timestamps"
    )
...
class SpeechProcessingJobResponse(BaseModel):
    """Response when submitting a speech processing job."""
...
class JobStatusResponse(BaseModel):
    ...
    processors: List[str] = Field(
        ...
        description="Processors being used for this job"
    )
...
class SpeechProcessingResultResponse(BaseModel):
    """Complete result response for speech processing."""
    ...
    processors_used: List[ProcessorResult] = Field(
        default_factory=list,
        description="Results from all processors"
    )
...
class HealthResponse(BaseModel):
    ...
    services: Dict[str, str] = Field(
        ...
        example={"redis": "connected", "nb_whisper": "loaded"}
    )