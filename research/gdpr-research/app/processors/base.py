...
class MemoryProcessor(ABC):
    """Base interface for all memory processors.

    This follows our processor plugin pattern where each input type
    (speech, documents, photos, etc.) implements this interface.
    """
    ...
    async def process(self, content: bytes, metadata: Dict[str, Any]) -> ProcessorResult:
        ...
        Args:
            content: Raw input data (audio file bytes for speech processor)
            metadata: Additional context and processing options

        Returns:
            ProcessorResult with structured output
        """
        pass
    ...
    def validate_input(self, content: bytes, metadata: Dict[str, Any]) -> bool:
        ...
        Args:
            content: Input data to validate
