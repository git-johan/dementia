...
async def load_nb_whisper_models():
    """Load all NB-Whisper models globally at startup."""
    global nb_whisper_models

    logger.info("Loading Norwegian Speech Recognition models: NB-Whisper...")
    ...
    try:
        ...
        logger.info(f"Norwegian Speech Recognition ready - NB-Whisper: {loaded_count} models")
        logger.info("Speech-to-text API ready for audio transcription")
...
async def _check_processor_status() -> bool:
    ...
    try:
        global nb_whisper_models
        # Consider system healthy if NB-Whisper is working
